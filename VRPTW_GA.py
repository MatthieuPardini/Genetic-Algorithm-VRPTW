...
# -*- coding: utf-8 -*-
"""
Algorithme génétique (avec contrainte de capacité --> solutions type VRP)
"""
...
def main():
    ...
    execution_unique()
    couleurs = ['blue','green','red','cyan','magenta','yellow','chocolate','slategray','tan','navy','indigo','pink']
    indice_couleur = -1
    for i in range(len(btournee)):
        indice_couleur = 0 if indice_couleur == len(couleurs) else indice_couleur+1
        for j in range(len(btournee[i])-1):
            afficher(btournee[i][j],btournee[i][j+1],couleurs[indice_couleur])
            

def afficher(i,j,couleur):
...
'''_____________________________________ AJOUTS POUR VRP ______________________________________'''

def split(tg):
    global dem; dem = np.full(n,1,dtype=float)
    global capa; capa = 7
    dmax = np.sum(dist)
...
def calculate_cost(route):
    ''' Nouvelle fonction calculate_cost adaptée au VRP'''
    cost,p = split(route)
    
    tournees = affiche_tournees(route,p)
    
    nouvelle_route = [0]
    for i in range(len(tournees)):
        for j in range(1,len(tournees[i])):
            nouvelle_route.append(tournees[i][j])
    nouvelle_route.append(0)
    
    times = time_windows(nouvelle_route)
    
    for i in range(1,len(times)):
        if times[i] > b[nouvelle_route[i]]:
            cost += penalite[nouvelle_route[i]]*(times[i]-b[nouvelle_route[i]])
...
'''_____________________________________ FIN DES AJOUTS POUR VRP __________________________________'''
...
def time_windows(route):  
    times = np.zeros(len(route),dtype=float)
    times[0]=0
    ...
    for gen in tqdm(range(0, nbgen)):
        ...
        if gen == 0:
            ...
            if costs[indice_best_cost] < bcosts[gen-1]:
                bsolution[gen] = population[indice_best_cost]
                bcosts[gen] = calculate_cost(bsolution[gen])                
            else:
...
def execution_unique():
    ...
    btournee = affiche_tournees(bsolution[nbgen-1],split(bsolution[nbgen-1])[1])
    ...
    worksheet.write('B5',str(btournee))
    ...
    print('\n\n*** Les résultats détaillés se trouvent dans le document Excel Résultats_GA_BONUS.xlsx ***')
